...
class XY_PROJECTED:
    def __init__(self,text):
        try:
            self.text = self.get_xyprojected_text(text)
    ...
    def get_xyprojected_text(self, text):
        #find the line number that contains the word *XY_PROJECTED
        line_num = get_line_numbers_containing_word(text, '*XY_PROJECTED')[0]

        #get the text between the two line numbers
        xyprojected_text = get_lines(text, [line_num])[0].split()
        
        return xyprojected_text
